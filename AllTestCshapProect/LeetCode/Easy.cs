...
namespace LeetCode {
    public class Easy {
        public static void Run () {
            {
                Console.WriteLine(MinDepth(new TreeNode(3) {
                    left = new TreeNode(9),
            ...
            if (false) {
                {
                    Console.WriteLine(HasPathSum(new TreeNode(5) {
                        left = new TreeNode(4) {
                    ...
                    Console.WriteLine(HasPathSum(new TreeNode(7) {
                        left = new TreeNode(0) {
                            left = new TreeNode(-1) {
                                right = new TreeNode(1) {
                                    left = new TreeNode(-7)
                ...
                IList<IList<int>> ret = ThreeSum(qu);
                ...
                foreach (var item in PlusOne(new int[] { 1 , 2 , 3 })) {
        ...
        public static bool HasPathSum (TreeNode root , int sum) {
            Func<TreeNode,int,bool> fun = null;
            fun = (node,n) => {
                if (node == null) {
                    return false;
                }
                ...
                if (node.left != null && fun(node.left , node.left.val + n)) {
                    return true;
                }
                if (node.right != null && fun(node.right , node.right.val + n)) {
                    return true;
                }
                return false;
            };
            return fun(root , root != null ? root.val : 0);
        }

        public static int MinDepth (TreeNode root) {
            int max = 0;
            Action<TreeNode , int> fun = null;
            fun = (node , n) => {
                if (node == null) {
                    max = n;
                    return;
            ...
            fun(root , root == null ? 0 : 1);
            return max;
        }

        //100
        public static bool IsSameTree (TreeNode p , TreeNode q) {
            List<TreeNode> pp = Check(new List<TreeNode>() { p });
            List<TreeNode> qq = Check(new List<TreeNode>() { q });

            if (pp.Count != qq.Count) {
                return false;
            }
            for (int i = 0 ; i < pp.Count ; i++) {
                if (pp[i] != null && qq[i] != null) {
        ...
        //100-TreeNode
        public static List<TreeNode> Check (List<TreeNode> _waits) {
            List<TreeNode> waits = new List<TreeNode>();
            List<TreeNode> nodes = new List<TreeNode>();
            foreach (var item in _waits) {
                nodes.Add(item);
                if (item != null) {
                    waits.Add(item.left);
                    waits.Add(item.right);
                }
            }
            bool isAllNull = true;
        ...
        public static string AddBinary (string a , string b) {
            ...
            while (true) {
                ...
                aa = a.Length - i >= 0 ? a[a.Length - i] : '0';
                bb = b.Length - i >= 0 ? b[b.Length - i] : '0';
                dd = aa == '1' && bb == '0' || aa == '0' && bb == '1' ? '1' : '0';  //计算前两个为1的情况
                if (cc == '1') {
                    dd = dd == '1' && cc == '0' || dd == '0' && cc == '1' ? '1' : '0';  //有进位时，再计算后两个为1的情况
                }
                if ((aa == '1' && bb == '1') || (aa == '1' && cc == '1') || (cc == '1' && bb == '1')) { //是否有进位
                    cc = '1';
                } else {
                    cc = '0';
                }
        ...
        public static int MaxSubArray (int[] nums) {
            ...
            int max = nums[0];
            while (right > 0) {
                int k = nums.Length - right;
                for (int i = 0 ; i <= k ; i++) {
                    int num = 0;
                    for (int ii = left + i ; ii < right + i ; ii++) {
                        num += nums[ii];
                    }
                    if (num > max) {
                        max = num;
                    ...
                    for (int j = i + 1 ; j < list.Count - 1 ; j++) {
                        for (int k = j + 1 ; k < list.Count ; k++) {
                            if (list[i] + list[j] + list[k] == 0) {
                                string key = $"{list[i]}{list[j]}{list[k]}";
        ...
        public static bool IsMatch (string s , string p) {
            ...
            for (int i = 0 ; i < pp.Count ; i++) {
                ...
                if (pp[i] != ss) {
                    if (pp[i] == '.') {
                        ...
                        if (maxCount > 0 && i + 1 < pp.Count && pp[i + 1] == c) {
                            maxCount--;
                        }
                        pp.RemoveAt(i);
                        ...
                        //Console.WriteLine(1111 + string.Join("" , pp.ToArray()));
                        break;
                    } else {
                        if (ss == ' ') {
                            for (int j = pp.Count - 1 ; j > s.Length - 1 ; j--) {
                                if (pp[j] == '*') {
                                    pp.RemoveAt(j);
                                    j--;
                                    pp.RemoveAt(j);
            ...
            string ppp = string.Join("" , pp.ToArray());

            if (ppp != s) {
                return IsMatch(s , ppp);
            }

            Console.WriteLine($"ssss == {s}");
            Console.WriteLine($"ssss == {p}");
            return true;
        }
        //9
        ...
        public static int Reverse (int x) {
            ...
            int xx = 0;
            int.TryParse(string.Join("" , ret.ToArray()) , out xx);
            return xx;
        }
        //4
        public double FindMedianSortedArrays (int[] nums1 , int[] nums2) {
            ...
            if (list.Count % 2 == 0) {
                return (list[list.Count / 2 - 1] + list[list.Count / 2]) / (double)2.0;
        ...
        public int LengthOfLongestSubstring (string s) {
            ...
            for (int i = 0 ; i < s.Length ; i++) {
                ...
                ret = list.Count > ret ? list.Count : ret;
            }
            return ret;
        }
        //2-obj
        ...
        public ListNode AddTwoNumbers (ListNode l1 , ListNode l2) {
            ...
            while (true) {
                if (_l1 == null && _l2 == null) {
                    if (yu != 0) {
                        ret.next = new ListNode(yu);
                    }
                    return head;
                }
                int a = _l1 == null ? 0 : _l1.val;
                int b = _l2 == null ? 0 : _l2.val;
                int sum = a + b + yu;
                yu = sum / 10;
                ...
                _l1 = _l1 == null ? null : _l1.next;
                _l2 = _l2 == null ? null : _l2.next;
            }
        }
        //1
        public static int[] TwoSum (int[] nums , int target) {
            ...
            //Enumerable.Range(0 , nums.Length).ToLookup(i => nums[i]);
            return null;
        }
        //48
